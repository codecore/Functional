...
namespace Tests {
    ...
    public class test_reduce_with_init : ISyncTestCase {
        private const string _Name = "test_reduce_with_init,F_reduce_init_T";
        ...
        private static bool _Run() {
            Boolean result = true;
            result = result && ((6 + 3 + 24 + 17) == F.reduce<int>(new List<int> { 3, 24, 17 }, (x, y) => x + y, 6));
            ...
            containsEven = F.reduce<int,bool>(test, (b, x) => (b || even(x)), false);
            containsOdd = F.reduce<int,bool>(test, (b, x) => (b || odd(x)), false);
        ...
        public test_reduce_with_init() {
            ...
            this.feature.Add(TestCoverage.F_reduce_init_T);
            this.feature.Add(TestCoverage.F_reduce_init_TU);

            this.coverage.Add(TestCoverage.F_T);
            this.coverage.Add(TestCoverage.F_reduce_init_T);
            this.coverage.Add(TestCoverage.F_reduce_init_TU);
        }
    }
    ...
    public class test_reduce_one : ISyncTestCase {
        ...
        private static bool _Run() {
            ...
            result = result && (12 == F.reduce<int>(seq, (x, y) => (x > y) ? x : y));
            result = result && (-4 == F.reduce<int>(seq, (x, y) => (x < y) ? x : y));
            result = result && ((16 + 18 - 2) == F.reduce<int>(new List<int>() { 16, 18, -2 }, (x, y) => x + y));
            return result;
        }
        private IList<int> coverage = new List<int>();
        private IList<int> feature = new List<int>();
        ...
        public test_reduce_one() {
            ...
            this.feature.Add(TestCoverage.F_reduce_naked_T);

            this.coverage.Add(TestCoverage.F_T);
            this.coverage.Add(TestCoverage.F_reduce_naked_T);
        }
    }
}
